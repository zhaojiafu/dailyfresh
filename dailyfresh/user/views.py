...
class IndexView(LoginRequiredMixin, View):
    ...
    def get(self,request):
        # 获取首页要显示的生鲜食物分类对象list
        good_types =GoodsType.objects.all().order_by('id')
        # 获取标题显示的生鲜食物对象list
        good_banners = IndexGoodBanner.objects.all().order_by('index')
        # 获取标题正在搞活动的生鲜食物对象
        good_promotion = IndexPromotionBanner.objects.all().order_by('index')
        # 首页分类商品展示图片模型类
        good_type_image_foods = IndexTypeGoodsBanner.objects.filter(display_type=1).order_by('index')
        # 首页分类商品展示标题显示模型类
        good_type_title_foods = IndexTypeGoodsBanner.objects.filter(display_type=0).order_by('index')
        # foods sku
        goods_sku = GoodsSKU.objects.all()
        # 测试代码
        # for i in food_type_image_foods:
        #     print(i.sku_id,2222)
        ...
        context1 = {
            ...
            'good_type_image_foods':good_type_image_foods,
            'good_type_title_foods':good_type_title_foods,
            'goods_sku':goods_sku,
        }
